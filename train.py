...
from datasets import PWDataset
from utils import *
...
print_freq = 2  # print training or validation status every __ batches
...
def main():
    ...
    # Custom dataloaders
    train_dataset = PWDataset(data_folder,
                                     split='train',
                                     keep_difficult=keep_difficult)
    val_dataset = PWDataset(data_folder,
                                   split='test',
                                   keep_difficult=keep_difficult)
...
def validate(val_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        for i, (images, boxes, labels, difficulties) in enumerate(val_loader):
            ...
            if i % print_freq == 0:
                print('[{0}/{1}]\t'
                      'Validation Loss {loss.val:.4f}\t'.format(i, len(val_loader),
                                                                      loss=losses))
